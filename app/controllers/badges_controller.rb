class BadgesController < ApplicationController
  ...
  def achieve(test_passage)
    BadgesRule.all.each do |rule|
      if rule.required_tests.include?(test_passage.test)
        badges_test_passage = rule.badges_test_passages.build(test: test_passage.test, user: test_passage.user)
  ...
  def check_user_attempts(rule, test_passage)
    rule.attempts.nil? ? true : (rule.attempts * rule.required_tests.count >= rule.attempts_by(test_passage.user).count)
  end
end
